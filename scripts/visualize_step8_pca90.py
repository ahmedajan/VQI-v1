...
VQI-S plots (1-15):
  ...
  2.  erc_fnmr10_all_providers_pca90.png
  3.  erc_by_dataset_pca90.png
  4.  ranked_det_all_providers_pca90.png
  5.  cross_system_erc_pca90.png
  ...
  13. erc_parallel_coordinates_pca90.png
  14. det_separation_strip_pca90.png
  15. analysis_pca90.md

VQI-V + Dual-score plots (16-25):
  16. erc_v_all_providers_pca90.png
  17. ranked_det_v_all_providers_pca90.png
  18. cross_system_v_erc_pca90.png
  19. combined_erc_comparison_pca90.png
  20. combined_erc_summary_table_pca90.png
  21. quadrant_eer_comparison_pca90.png
  22. quadrant_score_distributions_pca90.png
  23. quadrant_performance_table_pca90.png
  24. dual_score_value_assessment_pca90.png
  25. analysis_v_pca90.md

Usage:
    python scripts/visualize_step8_pca90.py --dataset voxceleb1
"""
...
# PCA-90% label used in all titles
PCA_LABEL = "(PCA-90%)"
...
def plot_erc_summary_table(eval_data, output_path):
    ...
    table.scale(1.2, 1.5)
    fig.suptitle(f"ERC FNMR Reduction Summary {PCA_LABEL} (VQI-S)", fontsize=13)
    save_fig(fig, output_path)
...
def plot_det_separation_table(eval_data, output_path):
    ...
    table.set_fontsize(9)
    table.scale(1.2, 1.5)
    fig.suptitle(f"Ranked DET EER by Quality Group {PCA_LABEL} (VQI-S)", fontsize=13)
    save_fig(fig, output_path)
...
def plot_timing_benchmark(output_path):
    ...
    ax.set_title(f"VQI Scoring Speed Benchmark {PCA_LABEL}")
    ax.legend(fontsize=8)
...
def plot_quadrant_performance_table(quadrant_data, output_path):
    ...
    table.scale(1.1, 1.4)
    fig.suptitle(f"Per-Quadrant Performance Table {PCA_LABEL}", fontsize=13)
    save_fig(fig, output_path)
...
def plot_dual_score_value_assessment(combined_data, quadrant_data, output_path):
    ...
    ax.set_title(f"FNMR Reduction at 20% Rejection {PCA_LABEL}")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3, axis="y")
    ...
    ax.set_ylabel("EER")
    ax.set_title(f"Q1 vs Q3 EER {PCA_LABEL}")
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3, axis="y")

    fig.suptitle(f"Dual-Score Value Assessment {PCA_LABEL}", fontsize=14, y=1.02)
    fig.tight_layout()
    save_fig(fig, output_path)


def write_analysis_s(eval_data, cross_data, bench_data, dataset, output_path):
    """Write analysis_pca90.md for VQI-S (output 15)."""
    lines = [
        f"# Step 8 (PCA-90%): VQI-S Evaluation Analysis - {dataset}",
        "",
        "## ERC Results",
        "",
    ]
...
def write_analysis_v(eval_v_data, combined_data, quadrant_data, dataset, output_path):
    """Write analysis_v_pca90.md for VQI-V + dual-score (output 25)."""
    lines = [
        f"# Step 8 (PCA-90%): VQI-V + Dual-Score Evaluation Analysis - {dataset}",
        "",
        "## VQI-V ERC Results",
        "",
    ]
...
def plot_cross_system_ranked_det(eval_data, dataset, data_dir, output_path, prefix="", suffix="_pca90"):
    ...
    fig.suptitle(f"Cross-System Ranked DET {PCA_LABEL} ({score_label}) -- {dataset}", fontsize=14, y=1.02)
    fig.tight_layout()
    save_fig(fig, output_path)
...
def plot_provider_performance_radar(eval_data, output_path):
    ...
    ax.set_ylim(0, 1)
    ax.set_title(f"Provider Performance Radar {PCA_LABEL} (VQI-S)", fontsize=14, y=1.08)
...
def plot_fnmr_reduction_forest(eval_data, output_path):
    ...
    ax.set_title(f"FNMR Reduction Forest Plot {PCA_LABEL} (FNMR=10%)")
...
def plot_erc_parallel_coordinates(eval_data, output_path):
    ...
    ax.set_ylabel("FNMR Reduction (%)")
    ax.set_title(f"ERC Parallel Coordinates {PCA_LABEL} (FNMR=10%)")
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)
    save_fig(fig, output_path)
...
def plot_det_separation_strip(eval_data, output_path):
    ...
    ax.set_xticklabels([PROVIDER_SHORT.get(p, p) for p in providers], rotation=15)
    ax.set_ylabel("EER")
    ax.set_title(f"DET EER by Quality Group {PCA_LABEL} (VQI-S)")
...
def plot_combined_erc_summary_table(combined_data, output_path):
    ...
    table.scale(1.2, 1.6)
    fig.suptitle(f"Combined ERC {PCA_LABEL}: FNMR Reduction at 20% Rejection", fontsize=13)
    save_fig(fig, output_path)
...
def plot_quadrant_score_distributions(vqi_scores_path, pair_scores_path, dataset, output_path):
    ...
    fig.suptitle(f"Score Distributions by Quality Quadrant {PCA_LABEL} -- {dataset}", fontsize=14, y=1.02)
    fig.tight_layout()
    save_fig(fig, output_path)
...
def plot_erc_ridgeline_across_datasets(datasets, output_path):
    ...
    ax.set_title(f"Cross-Dataset FNMR Reduction Comparison {PCA_LABEL} (FNMR=10%)")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3, axis="y")
    save_fig(fig, output_path)
    ...
    if args.dataset == "all":
        ...
        if len(multi_s_data) >= 2:
            ...
            plot_erc_ridgeline_across_datasets(multi_s_data,
                                                os.path.join(report_s_dir, "erc_ridgeline_across_datasets_pca90.png"))
        else:
...
def _generate_per_dataset(dataset):
    ...
    logger.info(f"Generating PCA-90% visualizations for {dataset}...")
    ...
    if eval_s_data:
        ...
        plot_erc_all_providers(eval_s_data, "fnmr_1pct",
                               os.path.join(report_s_dir, "erc_fnmr1_all_providers_pca90.png"),
                               "FNMR=1%")
        # 2. ERC at FNMR=10%
        plot_erc_all_providers(eval_s_data, "fnmr_10pct",
                               os.path.join(report_s_dir, "erc_fnmr10_all_providers_pca90.png"),
                               "FNMR=10%")
        # 4. Ranked DET
        plot_ranked_det_all_providers(eval_s_data, dataset, eval_s_dir, save_dir=report_s_dir)

        # 7. ERC summary table
        plot_erc_summary_table(eval_s_data, os.path.join(report_s_dir, "erc_summary_table_pca90.png"))

        # 8. DET separation table
        plot_det_separation_table(eval_s_data, os.path.join(report_s_dir, "det_separation_table_pca90.png"))

        # 11. Provider performance radar
        plot_provider_performance_radar(eval_s_data, os.path.join(report_s_dir, "provider_performance_radar_pca90.png"))

        # 12. FNMR reduction forest plot
        plot_fnmr_reduction_forest(eval_s_data, os.path.join(report_s_dir, "fnmr_reduction_forest_plot_pca90.png"))

        # 13. ERC parallel coordinates
        plot_erc_parallel_coordinates(eval_s_data, os.path.join(report_s_dir, "erc_parallel_coordinates_pca90.png"))

        # 14. DET separation strip
        plot_det_separation_strip(eval_s_data, os.path.join(report_s_dir, "det_separation_strip_pca90.png"))
    ...
    if eval_s_data:
        write_analysis_s(eval_s_data, cross_s_data, bench_data, dataset,
                         os.path.join(report_s_dir, "analysis_pca90.md"))
    ...
    if eval_v_data:
        ...
        plot_erc_all_providers(eval_v_data, "fnmr_10pct",
                               os.path.join(report_v_dir, "erc_v_all_providers_pca90.png"),
                               "VQI-V FNMR=10%")
    ...
    if cross_v_data:
        ...
        if eval_v_data:
            plot_cross_system_ranked_det(eval_v_data, dataset, eval_v_dir,
                                         os.path.join(report_v_dir, "cross_system_v_ranked_det_pca90.png"),
                                         prefix="_v")

    # 19. Combined ERC comparison
    if combined_data:
        plot_combined_erc_comparison(combined_data,
                                     os.path.join(report_v_dir, "combined_erc_comparison_pca90.png"))

        # 20. Combined ERC summary table
        plot_combined_erc_summary_table(combined_data,
                                        os.path.join(report_v_dir, "combined_erc_summary_table_pca90.png"))

    # 21. Quadrant EER comparison
    if quadrant_data:
        plot_quadrant_eer_comparison(quadrant_data,
                                     os.path.join(report_v_dir, "quadrant_eer_comparison_pca90.png"))
        ...
        plot_quadrant_performance_table(quadrant_data,
                                        os.path.join(report_v_dir, "quadrant_performance_table_pca90.png"))
    ...
    if combined_data and quadrant_data:
        plot_dual_score_value_assessment(combined_data, quadrant_data,
                                         os.path.join(report_v_dir, "dual_score_value_assessment_pca90.png"))

    # 25. Analysis V
    if eval_v_data:
        write_analysis_v(eval_v_data, combined_data, quadrant_data, dataset,
                         os.path.join(report_v_dir, "analysis_v_pca90.md"))

    logger.info(f"\nPCA-90% visualization complete for {dataset}.")
