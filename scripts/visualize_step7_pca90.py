"""
Step 7 Visualization: Model Validation Results (PCA-90%)
...
Usage:
    python visualize_step7_pca90.py
"""
...
def load_validation_data(score_type):
    ...
    # OOB convergence: check training dir first, then validation output dir
    conv_csv = os.path.join(train_dir, "oob_convergence.csv")
    if not os.path.exists(conv_csv):
        conv_csv = os.path.join(val_dir, "oob_convergence.csv")
    conv_df = pd.read_csv(conv_csv)

    conv_meta_path = os.path.join(train_dir, "oob_convergence_meta.yaml")
    if not os.path.exists(conv_meta_path):
        conv_meta_path = os.path.join(val_dir, "oob_convergence_meta.yaml")
...
def plot_score_distribution(data, prefix):
    ...
    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_score_distribution.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_genuine_cdf(data, provider, prefix):
    ...
    ax.set_ylabel("CDF")
    ax.set_title(f"CDF of Genuine Scores by VQI-{data['score_type'].upper()} Bin (PCA-90%) - {PROVIDER_LABELS.get(provider, provider)}")
    ax.legend(loc="lower right", fontsize=8)
    ax.grid(True, alpha=0.3)

    short = {"P1_ECAPA": "P1", "P2_RESNET": "P2", "P3_ECAPA2": "P3"}.get(provider, provider)
    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_genuine_cdf_{short}.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_oob_convergence(data, prefix):
    ...
    ax.set_title(f"VQI-{data['score_type'].upper()} OOB Error Convergence (PCA-90%)")
    ax.legend()
    ax.grid(True, alpha=0.3)

    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_oob_convergence.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_roc_curve(data, prefix):
    ...
    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_roc_curve.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_score_vs_genuine(data, prefix):
    ...
    fig.suptitle(f"VQI-{data['score_type'].upper()} Score vs Genuine Comparison Score (PCA-90%)", fontsize=12)
    fig.tight_layout()

    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_score_vs_genuine.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_dual_score_scatter():
    ...
    path = os.path.join(REPORT_DIR, "7_pca90_dual_score_scatter.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_quadrant_bar_chart():
    ...
    ax.set_title("Sample Distribution by Quadrant (PCA-90%)")
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{int(x):,}"))
    ...
    ax.set_title("Recognition Success Rate by Quadrant (PCA-90%)")
    ax.set_ylim(0, 105)

    fig.suptitle("Dual-Score Quadrant Analysis (PCA-90%)", fontsize=13)
    fig.tight_layout()

    path = os.path.join(REPORT_DIR, "7_pca90_dual_quadrant_bar.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_combined_rejection_curve():
    ...
    ax.set_title("Accuracy vs Rejection Threshold (PCA-90%)")
    ...
    ax.set_ylabel("Accuracy Among Accepted (%)")
    ax.set_title("Error-Rejection Curve (ERC) (PCA-90%)")
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)

    fig.suptitle("Combined Rejection Analysis (PCA-90%): VQI-S vs VQI-V", fontsize=13)
    fig.tight_layout()

    path = os.path.join(REPORT_DIR, "7_pca90_dual_combined_rejection.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def generate_validation_report(data, prefix, report_dir):
    """Generate validation_report.md for a score type (PCA-90%)."""
    m = data["metrics"]
    ...
    report = f"""# VQI-{score_type.upper()} Validation Report (PCA-90%)

## Model Parameters
- Algorithm: Random Forest (PCA-90% reduced features)
...
def generate_analysis_md():
    ...
    analysis = f"""# Step 7: Model Validation Analysis (PCA-90%)
...
def plot_score_by_quality_bin(data, prefix):
    ...
    ax.set_title(f"VQI-{data['score_type'].upper()} Score Distribution by Quality Bin (PCA-90%)")
    ax.grid(True, alpha=0.3, axis="y")

    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_score_by_quality_bin.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_score_by_dataset(data, prefix):
    ...
    ax.set_ylabel(f"VQI-{data['score_type'].upper()} Score")
    ax.set_title(f"VQI-{data['score_type'].upper()} Score by Dataset Source (PCA-90%)")
    ax.grid(True, alpha=0.3, axis="y")

    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_score_by_dataset.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_score_by_duration_bin(data, prefix):
    ...
    try:
        ...
        ax.set_title(f"VQI-{data['score_type'].upper()} Score by Speech Duration (PCA-90%)")
        ax.grid(True, alpha=0.3, axis="y")

        path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_score_by_duration_bin.png")
        fig.savefig(path)
        plt.close(fig)
        print(f"  Saved: {path}")
        return path
...
def plot_ridgeline_by_dataset(data, prefix):
    ...
    fig.suptitle(f"VQI-{data['score_type'].upper()} Score Ridgeline by Dataset (PCA-90%)", fontsize=12)
    plt.tight_layout()

    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_ridgeline_by_dataset.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_hexbin_vs_genuine(data, prefix):
    ...
    fig.suptitle(f"VQI-{data['score_type'].upper()} vs Genuine Score (PCA-90%) (Hexbin Density)", fontsize=12)
    fig.tight_layout()

    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_hexbin_vs_genuine.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_kde_distribution(data, prefix):
    ...
    ax.set_title(f"VQI-{data['score_type'].upper()} Score Distribution (PCA-90%): Class 0 vs Class 1")
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)

    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_kde_distribution.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_qq_uniformity(data, prefix):
    ...
    ax.set_title(f"QQ Plot: VQI-{data['score_type'].upper()} vs Uniform(0,100) (PCA-90%)")
    ax.legend()
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
    ax.set_aspect("equal")
    ...
    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_qq_uniformity.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_forest_plot(data, prefix):
    ...
    ax.set_title(f"Forest Plot: VQI-{data['score_type'].upper()} vs Genuine Score Correlation (PCA-90%)")
    ax.grid(True, alpha=0.3, axis="x")
    ax.invert_yaxis()
    plt.tight_layout()

    path = os.path.join(REPORT_DIR, f"7_pca90_{prefix}_forest_plot.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_dual_score_hexbin():
    ...
    ax.set_xlabel("VQI-S (Signal Quality)")
    ax.set_ylabel("VQI-V (Voice Distinctiveness)")
    ax.set_title(f"Dual Score Hexbin Density (PCA-90%) (N={len(df):,})")
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
    ax.grid(True, alpha=0.2)

    path = os.path.join(REPORT_DIR, "7_pca90_dual_score_hexbin.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def plot_dual_quadrant_table():
    ...
    ax.set_title("Dual-Score Quadrant Analysis (PCA-90%)", fontsize=12, pad=20)

    path = os.path.join(REPORT_DIR, "7_pca90_dual_quadrant_table.png")
...
def plot_dual_quadrant_genuine_violin():
    ...
    fig.suptitle("Genuine Score Distribution per Dual-Score Quadrant (PCA-90%)", fontsize=12)
    fig.tight_layout()

    path = os.path.join(REPORT_DIR, "7_pca90_dual_quadrant_genuine_violin.png")
    fig.savefig(path)
    plt.close(fig)
    print(f"  Saved: {path}")
    return path
...
def generate_analysis_v_md():
    ...
    text = f"""# Step 7: VQI-V Validation & Dual-Score Analysis (PCA-90%)
...
"""
    ...
    path = os.path.join(REPORT_DIR, "7_pca90_analysis_v.md")
    with open(path, "w", encoding="utf-8") as f:
        f.write(text)
    print(f"  Saved: {path}")
    return path
...
def main():
    print("=" * 60)
    print("Step 7 Visualization: Model Validation (PCA-90%)")
    print("=" * 60)
    ...
    try:
        ...
        # Validation report
        val_report_dir = os.path.join(PROJECT_ROOT, "reports", "validation_pca90")
    ...
    try:
        ...
        # Validation report
        val_v_report_dir = os.path.join(PROJECT_ROOT, "reports", "validation_pca90_v")
