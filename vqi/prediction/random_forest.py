"""
VQI-S Score Prediction via Random Forest (Sub-task 6.5).

Loads the trained VQI-S RF model and maps a feature vector to a [0-100] score.
...
def load_model(model_path: str) -> RandomForestClassifier:
    ...
    logger.info(
        "Loaded VQI-S RF model from %s (%d trees, %d features)",
...
def predict_score(clf: RandomForestClassifier, features: np.ndarray) -> int:
    """Map a feature vector to a VQI-S score in [0, 100].

    Args:
        clf: trained RandomForestClassifier
        features: (N_selected,) or (1, N_selected) feature vector

    Returns:
        Integer score in [0, 100]. Higher = better signal quality.
    """
    x = features.reshape(1, -1).astype(np.float32)
    proba = clf.predict_proba(x)[0]
    # Class 1 probability -> score
...
def predict_scores_batch(clf: RandomForestClassifier, features: np.ndarray) -> np.ndarray:
    """Map multiple feature vectors to VQI-S scores.

    Args:
        clf: trained RandomForestClassifier
        features: (N, N_selected) feature matrix
